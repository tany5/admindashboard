...
export class EditpreviousyeartesService {
  ...
  getAllFullLengthQuizById(prodId) {
    let headers = new  HttpHeaders ({'Content-Type': 'application/json'})
    return this.webService.get(`get_all_previous_year_quiz_by_id/${prodId}`, headers)    
  }
  editFullLengthQuiz(value) {
    let headers = new  HttpHeaders ({'Content-Type': 'text/plain'})
    return this.webService.post(`edit_previous_year_quiz`, {value}, headers)
  }
