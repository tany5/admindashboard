...
import { EditpreviousyeartesService } from './editpreviousyeartes.service'
@Component({
  selector: 'app-editpreviousyeartest',
  templateUrl: './editpreviousyeartest.component.html',
  styleUrls: ['./editpreviousyeartest.component.scss']
})
export class EditpreviousyeartestComponent implements OnInit {
  addQuiz: FormGroup
  ...
  constructor(private service: EditpreviousyeartesService, private route: ActivatedRoute, private _formbuilder: FormBuilder, private router: Router, private snackbar: MatSnackBar) { this.route.params.subscribe(
    (params: Params) => {
  ...
  this.service.getAllFullLengthQuizById(this.prodId).subscribe(
    (res) => {
      console.log(res)
      this.quizDetails = res['all_previous_year_quiz']
      ...
      this.year = this.quizDetails[0]['year']
