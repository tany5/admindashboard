...
export class EditsectionaltestService {
  ...
  getAllFullLengthQuizById(prodId) {
    let headers = new  HttpHeaders ({'Content-Type': 'application/json'})
    return this.webService.get(`get_all_sectional_quiz_by_id/${prodId}`, headers)    
  }
  editFullLengthQuiz(value) {
    let headers = new  HttpHeaders ({'Content-Type': 'text/plain'})
    return this.webService.post(`edit_sectional_quiz`, {value}, headers)
  }
